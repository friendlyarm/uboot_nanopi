...
#endif
	while (i--) {
		//if (0 == this->read_word(this->base + ONENAND_REG_CTRL_STATUS))
		interrupt = this->read_word(this->base + ONENAND_REG_INTERRUPT);
		if (interrupt & flags)
			break;
		udelay(100);
 ...
 * complete the work
 */
static onenand_interrupt(int irq, void *data)
{
...
{
	struct onenand_chip *this = mtd->priv;
	unsigned char *readp = this->page_buf;
	...
	this->read_bufferram(mtd, ONENAND_SPARERAM, (unsigned char *)readp, column, len);
